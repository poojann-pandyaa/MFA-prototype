"""
RawNet2 Architecture for Audio Anti-Spoofing / Presentation Attack Detection.

RawNet2 implementation is based on:
https://github.com/NTU-ROSE/RawNet2
and the official ASVspoof 2021 baseline:
https://github.com/asvspoof-challenge/2021/tree/main/LA/Baseline-RawNet2

Original Author: Hemlata Tak
Email: tak@eurecom.fr
Credits: Jose Patino, Massimiliano Todisco, Jee-weon Jung
Affiliation: EURECOM, 2021
Citation:
  Tak, Hemlata, et al. "End-to-End anti-spoofing with RawNet2."
  IEEE ICASSP 2021, pp. 6369-6373.
"""
...
class SincConv(nn.Module):
    ...
    def __init__(self, device, out_channels, kernel_size, in_channels=1, sample_rate=16000,
        ...
        # Force filter size to be odd for symmetric filters
        if kernel_size % 2 == 0:
        ...
        # Maintain exact upstream non-persistent buffers so load_state_dict matches perfectly
        self.hsupp = torch.arange(-(self.kernel_size - 1) / 2, (self.kernel_size - 1) / 2 + 1)
        self.band_pass = torch.zeros(self.out_channels, self.kernel_size)

    def forward(self, x):
        hsupp_np = self.hsupp.cpu().numpy()
        hamming_window = np.hamming(self.kernel_size)

        for i in range(len(self.mel) - 1):
            fmin = self.mel[i]
            fmax = self.mel[i + 1]
            hHigh = (2 * fmax / self.sample_rate) * np.sinc(2 * fmax * hsupp_np / self.sample_rate)
            hLow = (2 * fmin / self.sample_rate) * np.sinc(2 * fmin * hsupp_np / self.sample_rate)
            hideal = hHigh - hLow

            self.band_pass[i, :] = Tensor(hamming_window * hideal)
...
class RawNet(nn.Module):
    ...
    def forward(self, x, y=None):
        # Expected input shape: (batch_size, nb_samp)
        nb_samp = x.shape[0]
        len_seq = x.shape[1]
